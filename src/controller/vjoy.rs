...
static VJOY: LazyLock<Result<Mutex<VJoy>, String>> = LazyLock::new(|| {
    VJoy::from_default_dll_location()
        .map(Mutex::new)
        .map_err(|err| err.to_string())
});

pub fn set_device_id(id: u8) -> anyhow::Result<()> {
...
impl Controller {
    ...
    pub fn new(_device_name: &str) -> anyhow::Result<Self> {
        let vjoy = VJOY
            .as_ref()
            .map_err(|err| anyhow!("Failed to init vjoy: {err}"))?;
